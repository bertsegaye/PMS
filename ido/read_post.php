...
<div class="container-fluid">
                ...
                <?php
                $count = $con->query("SELECT COUNT(*) as total FROM `announcement` WHERE
                sm_status='1'") or die(mysqli_error()); $countt =
                $count->fetch_array(); ?>
...
<section id="contact">
          ...
          <?php 
           if (isset($_GET['post'])){
                 ...
                 $id=$_GET['post'];
                 $setread=mysqli_query($con, "UPDATE announcement SET ido_status=0 WHERE id='$id'");
           //$query = $con->query("SELECT * from users_info") or die(mysqli_error());
           $query = $con->query("SELECT * from announcement WHERE id='$id' ") or die(mysqli_error());
                          while($fetch = $query->fetch_array()){
                          ?>
                          <tr
                         <?php if ($fetch['ido_status'] == 1) {
